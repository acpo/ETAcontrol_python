...
## 03/2023 put dynamic COM port selector to avoid hard coding.
## 02/2024 updated scalar extraction to comply with Numpy > 1.25 standards
## 01/2025 Separated dry/pyrolyze step from atomize/clean step.  Separate memory slots must be used.
##          The Measure button calls the two memory slots in sequence.  
...
import tkinter.ttk as ttk  #necessary for combo box in Serial selection
...
try:
    ...
    exit()  # commented out to allow testing without a serial powersupply attached
except:
...
#GUI entries
        ...
        self.PS_button = tk.Button(self.menu_left_lower, text='Measure')
        self.PS_button.grid(column=1, row=3)
        self.PS_button.bind('<ButtonRelease-1>', self.PS_dry)
        self.PSdryslot = 1
        self.PS_dryslot_label = tk.Label(self.menu_left_lower, text='PS drying memory slot', relief = 'ridge')
        self.PS_dryslot_label.grid(column=0, row=4)
        self.PS_dryslot = tk.Spinbox(self.menu_left_lower, from_=0, to=7, textvariable=self.PSdryslot, width=3)
        self.PS_dryslot.grid(column=1, row=4)
# new buttons
        self.PSatomslot = 2
        self.PS_atomslot_label = tk.Label(self.menu_left_lower, text='PS atomize memory slot', relief = 'ridge')
        self.PS_atomslot_label.grid(column=0, row=6)
        self.PS_atomslot = tk.Spinbox(self.menu_left_lower, from_=0, to=7, textvariable=self.PSatomslot, width=3)
        self.PS_atomslot.grid(column=1, row=6)
# end new

        self.PS_emergencystop_button = tk.Button(self.menu_left_lower, text='Stop PS', bg='red', fg='white', font='bold')
        self.PS_emergencystop_button.grid(column=1, row=10)
        ...
        self.PSconnect.grid(column=0, row=10, pady=10)#, columnspan=1)
        self.PSconnect.bind('<ButtonRelease-1>', self.Connect_PS)
        
        tk.Label(self.menu_left_lower, text = "COM port", relief = 'sunken').grid(column=0, row=11)
        ...
        self.ports_box.grid(column = 0, row = 12)
...
#Create the line artist objects for Blit
    ...
    def TimeLimit_change(self, event):
        ...
        if self.DisplayCode == 0:
            ...
            self.canvas.draw()
## dynamic Serial selection
    def Connect_PS(self, event):
        if(ser.isOpen() == False):  # check if serial port is open
            try:
                ser.open() #without explicit open/close we just assume ser is open and blocking other programs?
    ...
    def on_selectComm(self, event):
        ...
        print(ser)
## start Power Supply
    def PS_dry(self, event):  # runs power supply for drying and pyrolysis
        gc.collect()
        self.DisplayCode = 1 # simulates button press to go to time series mode
        self.DisplayMode(event)
        self.btn.config(text='Running')
        GO_string = "R " + self.PS_dryslot.get()  # sends 'R' as in Run and the slot number formatted for MasTech powersupply
        ...
        return self.PS_go()  #calls the atomization program
...
##        self.DisplayMode(event)
##        self.btn.config(text='Running')
        GO_string = "R " + self.PS_atomslot.get()  # sends 'R' as in Run and the slot number formatted for MasTech powersupply
        self.PStext.insert(tk.END, "sent: ")
        self.PStext.insert(tk.END, GO_string)
        self.PStext.insert(tk.END, "\n")
        #send serial command to start power supply
    ...
    def PS_EmergencyStop(self, event):
        ...
        ser.flush()  # avoids accidental stray instructions
        self.readSerial()
## start add for dynamic Serial selection        
def scanSerial():
