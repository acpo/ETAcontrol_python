    ...
    def readSerial(self):
        ...
        while True:
            ...
            if len(serialdata) > 0:
                ...
                serialdata = str(serialdata).replace("\r","") #removes returns
                self.SerialBuffer += "\n" # add the newline to the buffer
                self.SerialBuffer += serialdata
                self.PStext.insert(tk.END, self.SerialBuffer)
                self.SerialBuffer = "" # empty the buffer
            else:
                self.SerialBuffer += "no data \n"
                self.PStext.insert(tk.END, self.SerialBuffer)
                self.SerialBuffer = "" # empty the buffer
                break
        ser.close()
