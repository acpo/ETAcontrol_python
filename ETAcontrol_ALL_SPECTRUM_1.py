## This is a WHOLE SPECTRUM ACQUISITION VERSION
#  No visual feedback during acquisition to save resources for better timing.
#  That means Integration Time down to Hardware limit is possible -- typically 5 ms
#  
...
#GUI entries
        ...
        self.more_title = tk.Label(self.some_title_frame, text="WHOLE SPECTRUM COLLECTION -- NO LIVE DISPLAY IN TIME SERIES MODE", bg="#dfdfdf")
        self.more_title.pack()
        self.more2_title = tk.Label(self.some_title_frame, text="Set 'Time series length' as start to end of atomization", bg="#dfdfdf", fg="red", font='bold')
        self.more2_title.pack()
...
#Create the line artist objects for Blit
    ...
    def TimeLimit_change(self, event):
        ...
        if self.DisplayCode == 0:
            ...
            self.canvas.draw()
## start addition for dynamic Serial selection
...
def saveFile(data_time, alldata, atom_time):
    ...
    if not filenameforWriting:
        ...
        allfile = str(path_ext[0] + "all" + path_ext[1])
        specmodel = spec.model
        allheader = "# Spectrometer = " + specmodel + "\n# Spectral data "
