...
import time  #needed for Serial comms
...
#Create the line artist objects for Blit
        ...
        self.linedata, = self.ax1.plot([], [], '.', color='red')#, markevery=20)
        self.bkgdata, = self.ax1.plot([], [], '.', color='green')#, markevery=20)
    ...
    def update_graph(self):
        ...
        while self.DisplayCode == 1:
            ...
            self.ax1.set_ylim(self.ymin*0.8, self.max_intensity) #set to max to accommodate any result
            self.canvas.draw()
            gc.collect()
            xdata = []
            linedata = []
...
#demo of a workable data saving routine
    ...
    def PS_go(self, event):
        ...
        self.btn.config(text='Running')
        GO_string = "R " + self.PS_slot.get()  #sends 'R' as in Run and slot number formatted for MasTech powersupply
    ...
    def readSerial(self):
        ...
        if ser.inWaiting() > 0:
            ...
            self.PStext.insert(tk.END, data_str) #insert received data in box
            self.PStext.insert(tk.END, "\n")
    ...
    def writeSerial(self, event):
        ...
        self.PStext.insert(tk.END, self.PSentry.get()) #to call the box from outside of class, we need to use a def so call App.def....
        self.PStext.insert(tk.END, "\n")
        datatosend = self.PSentry.get()
        self.PSentry.delete(0, 'end')
