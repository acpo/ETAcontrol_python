...
#Create the line artist objects for Blit
    ...
    def on_click(self):
        ...
        if self.DisplayCode==1:
            self.ani = FuncAnimation(
                ...
                #init_func=init,
                interval=1, #(self.IntTime/1000)-1,
            ...
            self.linedat = []
            self.bkgdat = []
            self.basedat = []  #base data is calculated but not drawn to save on drawing overhead and gain speed
            self.linedata.set_data(self.xdata, self.linedat)
            self.bkgdata.set_data(self.xdata, self.bkgdat)
            ...
            self.index1 = np.where(self.wavelengths == float(self.wavelength1))[0]  #array index of the chosen wavelength
            self.index2 = np.where(self.wavelengths == float(self.wavelength2))[0]
            self.index3 = np.where(self.wavelengths == float(self.wavelength3))[0]
            self.starttime = time.time()
            self.ani = FuncAnimation(
                fig=self.fig,
                func=self.update_graph,
                frames = self.timelimit * int(1000/self.IntTime*1000)+1,
                interval=1, #self.IntTime/1000,
                cache_frame_data=False,
                repeat=False,
                blit=True)
            print("per point = ", str((time.time()-self.starttime)/(self.timelimit * int(1000/self.IntTime*1000))))    
    ...
    def update_graph(self, frame):
        ...
        while self.DisplayCode == 1:
            ...
            while frame < self.timelimit * int(1000/self.IntTime*1000):
                self.ydata = np.array(get_intensities()) # gets full data
                self.xdata.append(time.time() - self.starttime) #correctly appends on each cycle
                self.linedat.append(float(self.ydata[self.index1]))  #append doesn't work on np.arrays
                self.bkgdat.append(float(self.ydata[self.index2]))
                self.basedat.append(float(self.ydata[self.index3]))
                self.linedata.set_data(self.xdata, self.linedat)
                self.bkgdata.set_data(self.xdata, self.bkgdat)

                return self.linedata, self.bkgdata,
                #self.basedata.set_data(xdata, basedata)
                #self.bm.update()  #replaced "return" with blit manager call
                
            #print("per point = ", str((time.time()-self.starttime)/(self.timelimit * int(1000/self.IntTime*1000))))
            print(np.diff(self.xdata))
