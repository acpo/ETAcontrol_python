...
#Create the line artist objects for Blit
        ...
        self.basedata, = self.ax1.plot([], [], lw=1, color='purple')
    ...
    def update_graph(self):
        ...
        while self.DisplayCode == 1:
            #        self.bm = BlitManager(self.fig.canvas, [self.line, self.waveline1]) # so we need to remind blit on return from other mode??
            ...
            #print("per point = ", str((time.time()-starttime)/(self.timelimit * int(1000/self.IntTime*1000))))
            #print(np.diff(xdata))
            self.btn.config(text='Start')
#demo of a workable data saving routine
            #xdata=np.asarray(xdata)
            #linedata=np.asarray(linedata)
            #saveFile(xdata, linedata)
...
def saveFile(data_to_write1, data_to_write2):
    filenameforWriting = asksaveasfilename()
    np.savetxt(filenameforWriting, (data_to_write1, data_to_write2), delimiter=',')
