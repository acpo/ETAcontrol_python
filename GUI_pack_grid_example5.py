...
#my settings for the axes. Get these from the spectrograph data
    ...
    def start(self):
        ...
        self.ani = animation.FuncAnimation(
            self.fig, #object to draw
            self.update_graph, #function called on each frame
            frames=self.points,
            interval=self.IntTime/1000, #IntTime is natively microseconds, Interval is milliseconds
            repeat=False)
        self.running = True
        self.btn.config(text='Pause')
        self.ani._start()
    ...
    def update_graph(self, i):
        ...
        if self.DisplayCode == 0:
            xdata = []
            linedata = np.zeros(self.timelimit*int(1000/self.IntTime*1000)) #simulate 10 ms integration time
            linetemp = []
            bkgdata = np.zeros(self.timelimit*int(1000/self.IntTime*1000))
            bkgtemp = []
            starttime = time.time()
            for zzz in range(self.timelimit * int(1000/self.IntTime*1000)):
                ...
                index1 = np.where(self.wavelengths == float(self.wavelen1box.get()))[0]
                linetemp.append(float(ydata[index1]))
                linedata = np.vstack((xdata, linetemp))
                self.line.set_data(linedata)
                index2 = np.where(self.wavelengths == float(self.wavelen2box.get()))[0]
                bkgtemp.append(float(ydata[index2]))
                bkgdata = np.vstack((xdata, bkgtemp))
                self.line.set_data(bkgdata)
            self.canvas.draw()
            #print(xdata)
            print(np.mean(np.diff(xdata)))
            print(np.std(np.diff(xdata)))
            # 1.3% timestamp standard deviation on 5 second acquisition at 25ms IntTime
# with drawing outside of the loop, the times are correct.
...
#  is TOO SLOW
    ...
    def RescaleY(self, event):
        ...
        self.ymin = np.around(min(ydata)*0.8, decimals=2)
        self.ymax = np.around(max(ydata)*1.1, decimals=2)
